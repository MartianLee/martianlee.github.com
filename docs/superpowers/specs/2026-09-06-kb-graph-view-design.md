...
- `KBGraphView`가 `KBShell`을 렌더링합니다. `sidebar`는 기존 `KBSidebar`, `main`은 컨트롤 줄 + SVG, `context`는 `KBGraphPanel`, breadcrumb은 "Graph", 상태바는 `● KB · 100 notes · 9 topics · 52 linked · 48 orphans · Esc to deselect · ⌘K to search`.
...
- 검색: `⌘K`로 포커스. `title`, `shortTitle`, slug, 태그에 부분 일치. 일치 노드에 액센트 테두리, 나머지 `dim`. Esc로 비움.
