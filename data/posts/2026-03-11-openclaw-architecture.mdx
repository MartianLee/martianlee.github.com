...
이 점에서 [Hermes](/kb/2026-05-13-hermes-agent-architecture)와 방향이 갈립니다. OpenClaw는 Claude Code나 Codex 같은 외부 코딩 에이전트를 호출하는 절차를 스킬로 가르치는 쪽에 가깝고, Hermes는 `delegate_task`로 Hermes 내부의 서브에이전트를 직접 띄우는 구조를 함께 갖고 있습니다.
