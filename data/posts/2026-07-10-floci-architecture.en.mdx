...
![A real session bringing Floci up with `docker compose up` and checking it via the AWS CLI](/static/images/floci-start.png)

> **A note from actually running it.** My first attempt failed. The reason was mundane: the Docker daemon (OrbStack) simply wasn't running on my Mac. Once I started OrbStack and brought it back up, it came online immediately. Given that Floci ships as a container and spins up real Docker for some services like Lambda and RDS ([§10](#10-it-emulates-for-real)), it's worth remembering up front that the Docker daemon has to be running.
...
![Checking Floci's status after bringing it up](/static/images/floci-status.png)
