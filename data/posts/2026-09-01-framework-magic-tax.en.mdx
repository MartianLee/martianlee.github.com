...
5. [The abstraction cost](#5-the-abstraction-cost)
...
I will call that difference the **abstraction cost**, the far end from a zero-cost abstraction. Rails hands you migrations, callbacks and `post.user` in a single line, and this is what that convenience spends on every request.
...
flowchart LR
    ...
    F --> GATE
    B --> GATE
    GATE --> T["the difference = abstraction cost<br/>CPU per request · throughput · memory"]
```
...
<MagicTaxStack lang="en" />

Divided inside each pair, that is the abstraction cost:
...
**③ And it isn't a Ruby problem.** Django's ORM costs ×6.14. **The amount of magic is the price**, and the two frameworks that do the most of it cost the most. Go's cost is small not because Go is fast but because GORM has no callbacks, no dirty tracking, and no generated association methods.
...
**Nothing about developer productivity.** The entire reason frameworks exist is missing from every number here. An abstraction cost of ×7.67 is not an argument against magic; it is **the price tag on it**. Buying with the tag visible differs from buying blind, and that difference is all this post is trying to make.
