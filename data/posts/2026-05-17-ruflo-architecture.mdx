...
| 계층    | 역할                                                                        |
| ------- | --------------------------------------------------------------------------- |
| CLI     | `ruflo init`, `swarm`, `memory`, `hooks`, `mcp` 같은 사용자 명령 인터페이스 |
...
| 글                                                       | 중심 문제                                           | Ruflo와의 관계                                                                                                         |
| -------------------------------------------------------- | --------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| [Superpowers](/kb/2026-04-18-superpowers-architecture)   | 에이전트에게 개발 절차를 강제하는 skill 문서 시스템 | Ruflo plugin/skill 영역과 비슷하지만 Ruflo는 더 큰 실행 인프라를 포함합니다.                                           |
...
| 가볍게만 쓸 수 있나요?             | Claude Code plugin marketplace로 slash command, agent, skill만 설치하는 경로가 있습니다.             |
| CLI 밖에서도 쓰나요?               | RuVocal Web UI와 Goal Planner UI가 같은 MCP/agent 개념을 제품 영역으로 확장합니다.                   |
...
이 구조는 현실적인 절충입니다. 모든 사용자가 처음부터 `.claude-flow/`, hooks, MCP, daemon을 원하지는 않습니다. 그래서 Ruflo는 제품 진입 경로를 두 단계로 둡니다.
...
장점은 Claude Code가 MCP tool만 알면 된다는 것입니다. 내부가 파일 저장소든 AgentDB든 worker daemon이든 호출 인터페이스는 도구 이름으로 통일됩니다.
...
CLI의 `memory-tools.ts`에는 나름의 현실적인 복잡도가 있습니다.
...
이 설계는 Superpowers와 닮았습니다. `SKILL.md`와 agent definition으로 에이전트 행동을 바꾸는 문서 기반 인터페이스를 사용합니다. 다만 Ruflo plugin은 여기에 MCP tool, smoke script, package compatibility, namespace convention까지 함께 묶습니다.
...
smoke script를 contract로 두기도 합니다. 즉, plugin README는 단순 사용 설명서가 아니라 **호환성 계약 문서**에 가깝습니다.

이 점은 Ruflo의 중요한 설계 철학처럼 보입니다.

> 기능을 하나의 거대한 CLI help로만 노출하지 않고, Claude Code plugin 단위로 "설치 가능한 업무 묶음"을 만듭니다.

이렇게 하면 사용자는 `ruflo-core + ruflo-swarm + ruflo-testgen + ruflo-ddd`처럼 목적별 stack을 조합할 수 있습니다. 반대로 개발자는 각 plugin의 smoke script와 README로 노출 영역을 regression-protect할 수 있습니다.

## 14. RuVocal과 Goal UI: CLI 밖의 제품 영역

Ruflo는 CLI와 plugin만 있는 프로젝트가 아닙니다. `ruflo/src/ruvocal/`과 `v3/goal_ui/`는 Ruflo가 제품 영역을 Web으로 넓히려는 흔적입니다.
...
셋째, verification witness입니다. `verification/` 아래에는 platform별 manifest, capabilities, results가 있습니다. README와 plugin 문서는 smoke test와 witness manifest를 계속 언급합니다. Ruflo처럼 다루는 범위가 넓은 프로젝트에서는 "어디까지 실제로 동작한다고 주장할 수 있는가"가 중요하기 때문에, 이런 증거 계층을 따로 둔 점은 좋게 보입니다.
...
1. `README.md`
   ...
   제품이 무엇을 주장하는지 먼저 봅니다. 단, 마케팅 수치는 그대로 믿기보다 어떤 코드 영역으로 이어지는지 확인하는 용도로 읽는 편이 좋습니다.
...
RuVocal은 MCP server group, parallel tool calls, bring-your-own MCP server, MongoDB persistence를 품고 있습니다. CLI 기능을 그대로 Web에 노출하려는 제품형 인터페이스입니다.
...
5. 제품 영역은 terminal에서 Web UI로 확장됩니다.
