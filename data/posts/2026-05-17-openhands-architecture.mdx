...
이 구조는 "agent가 직접 GitHub token을 들고 PR을 연다"가 아닙니다. app server가 인증 문맥과 제품 문맥을 갖춘 상태에서 MCP tool을 제공합니다. GitLab MR, Bitbucket, Azure DevOps도 같은 방향으로 확장됩니다.
...
`frontend`는 React Router 7 기반 SPA입니다. route 목록을 보면 제품이 다루는 범위가 잘 보입니다.
...
흥미로운 점은 frontend가 app server API와 agent-server runtime API를 모두 이해한다는 점입니다. app server에서 conversation metadata를 받고, conversation URL로 sandbox 내부 agent-server endpoint도 호출합니다.
...
이 구조는 Superpowers와 닮았습니다. Superpowers가 skill 문서로 agent 절차를 강제했다면, OpenHands는 제품 안에서 repository, organization, built-in skill을 합쳐 agent context를 구성합니다.
...
3. `openhands/app_server/app.py`와 `v1_router.py`로 FastAPI가 노출하는 부분을 봅니다.
